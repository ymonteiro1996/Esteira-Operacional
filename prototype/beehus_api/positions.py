...
from .client import request, request_multipart, bind_session_id, id_sessao_atual
...
def _get_positions_chunked(path, base_params, wallet_ids, timeout):
    ...
    [2026-08-06, pedido do usuário: "mais de uma pessoa ao mesmo tempo";
    CORRIGIDO no mesmo dia — achado em produção: "cannot enter context: ...
    is already entered"] Cada worker chama client.bind_session_id(sid) DENTRO
    da própria thread, com o sid capturado ANTES do fan-out
    (id_sessao_atual()) — a 1ª versão reusava um único `contextvars.Context`
    (copy_context().run(...)) entre todos os blocos concorrentes, mas um
    Context não pode ser "entrado" por 2 threads ao mesmo tempo. Propagar só
    o VALOR do sid evita a colisão (cada thread nova já nasce com seu próprio
    contexto isolado)."""
