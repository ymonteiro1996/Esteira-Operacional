...
> - Também corrigido, no mesmo lote: `index.js::init()` agora dispara um "Atualizar" automático assim que a página carrega (mesma rota, mesma garantia de frescor) — o usuário não depende mais só do `snapshot.json` estático (congelado no boot) nem precisa lembrar de clicar "Atualizar" manualmente ao abrir a tela. **[REVERTIDO 2026-09-22 — ver a nota do seletor de empresa, no fim desta seção: o refresh automático saiu e a 1ª consulta voltou a ser sempre um clique.]**
...
> **[REVISADO 2026-09-22, pedido do usuário: "não atualizar ao executar a primeira vez, permitir selecionar data e company e depois dar um atualizar clickando no botão" — escopo por empresa e fim do refresh automático]** Com o clique custando ~9 minutos (nota de 15/09 abaixo), duas coisas que eram convenientes viraram estorvo: a tela disparava uma consulta sozinha ao carregar (e outra ao colar o token), sempre na janela default e em TODAS as empresas — quem queria outra data/escopo esperava essa primeira terminar, ou disputava com ela.
>
> - **Nada mais consulta sozinho.** `index.js::init()` só deixa a tela pronta (preenche De/Até via `/api/janela-padrao`, preenche o seletor de empresas e escreve o convite "Escolha a data de referência e a empresa e clique em ↻ Atualizar" na `.atualizar-msg`); `salvarTokenBeehus()` (`beehus_token.js`) idem, depois que o token é aceito. A 1ª consulta é SEMPRE um clique. O 1º acesso continua com a tela interativa e sem dado velho disfarçado de atual — `static_files()` já serve um snapshot vazio quando o `snapshot.json` em disco falta ou está com data de referência velha (notas de 31/08 e 11/09 no `prototype/CLAUDE.md`).
> - **Seletor "Empresa" na toolbar** (`#empresa-atualizar`, ao lado dos campos de data), preenchido por `GET /api/empresas` (rota nova, fina: `db.listar_empresas()` = 1 chamada a `list_companies()`; de propósito NÃO usa `carregar_colecoes_pequenas()`, que também busca wallets/groupings/explosão de todas as empresas e leva dezenas de segundos a frio). A escolha vai como `company_id` em `GET /api/atualizar` e é aplicada em `_montar_snapshot()` logo depois do passo `[1/6]` (`_filtrar_cadastro_por_empresa`), ANTES do fan-out da esteira — então o clique consulta só as datas × 1 empresa, uma fração das chamadas de "Todas as empresas". Vazio = todas, comportamento idêntico ao de antes do campo existir.
> - **Limites aceitos:** (a) com filtro ativo, `mapear_carteiras_compradas()` enxerga só as carteiras daquela empresa, então o cruzamento "comprada por carteira de OUTRA empresa" (13/08) não é marcado enquanto o filtro estiver ligado; (b) o seletor lista as empresas que o TOKEN enxerga (~19), não só as ~5 com carteira no `TemplateCarteiras.xlsx` — saber isso exigiria justamente a chamada cara que a rota evita; escolher uma empresa sem carteira no cadastro devolve a grade vazia com a mensagem "Nenhuma carteira do cadastro em ...".

> **[REVISADO 2026-09-15 — rate limit da API Beehus: freio compartilhado, cache congelado por execução e progresso na tela]** A API nova passou a aplicar rate limit (ver `prototype/CLAUDE.md`, nota de 2026-09-11) e o custo por clique em "Atualizar" mudou de ordem de grandeza: **683 chamadas upstream, 126 delas 429 (18%), ~9 minutos por clique** — medido com `beehus_api.client.enable_timing()` numa janela de 01/09..09/09. Três consequências que a estratégia de otimização desta seção não previa, todas corrigidas:
>
> - **`CacheTTL` ganhou `congelado()`** (`cache.py`): o TTL de 120s das coleções pequenas pressupunha que quem usa o cache termina em muito menos que isso. Com o build em ~9min, o cache expirava DENTRO da execução que o tinha populado e o registry inteiro era rebuscado — no log de instrumentação, toda chamada `/beehus/securities/<id>` (1 por ativo de explosão) aparecia 2x no mesmo clique. `montar_snapshot()` agora roda dentro de `cache_ttl_colecoes.congelado()`, que suspende a expiração **só para o que foi guardado depois do congelamento começar** — a 1ª leitura de cada execução continua buscando dado fresco, e só o reaproveitamento dentro da própria execução deixa de expirar. Não muda nada da regra `forcar_atualizacao=True` acima: o cache POR DATA (`CachePorData`) continua sendo invalidado a cada clique.
> - **Backoff de 429 deixou de ser privado de cada thread** (`beehus_api/client.py`): as 6 threads do fan-out tomavam 429 quase juntas, dormiam o mesmo tempo (sem jitter) e voltavam a disparar juntas. Agora há uma pausa ÚNICA do processo (`_rate_limit_pausa_ate`, respeitada por toda chamada antes de sair) e a espera individual sai com jitter de ±50%. O 429 do servidor vem como página HTML, **sem `Retry-After` nem `X-RateLimit-*`** — pedir esses headers ao time do backend é o que permitiria o cliente se auto-regular de verdade em vez de adivinhar o backoff.
> - **Progresso por sessão** (`progresso_atualizacao.py` + `GET /api/atualizar/progresso` + `static/js/controle_cargas/progresso.js`): com 9 minutos de espera, o botão travado em "Atualizando..." era indistinguível de travado de verdade, e quem clicava de novo DOBRAVA a carga em cima do mesmo rate limit. `montar_snapshot()` publica a etapa corrente e o fan-out de `db.py` publica quantas consultas já terminaram; a tela lê de 2 em 2s. É informativo apenas — sem sid (CLI/boot) vira no-op e nenhum erro dali derruba um build.
>
> **Ainda em aberto**: o custo de 683 chamadas por clique continua alto por desenho (`forcar_atualizacao=True` refaz a janela toda). Se o rate limit apertar mais, os caminhos seriam negociar cota/headers com o time da API, ou rever a decisão de 2026-07-27 de sempre forçar a janela inteira.
...
> **[REVISADO 2026-09-22, relato do usuário: "um colega ainda não está aparecendo o responsável e comentários" — com o colega na MESMA data de referência]** Duas correções na camada de CAMINHO, mais um diagnóstico na tela:
>
> - **A pasta compartilhada agora se acha sozinha** (`utils/caminhos.py::_procurar_data_dir_compartilhado`, nova). O caminho era um literal único com o nome que o OneDrive dá à biblioteca nesta máquina; em outra ele muda (Windows em inglês sincroniza "... - Documents", biblioteca no OneDrive pessoal fica um nível acima, pasta pode ter sido renomeada) e o app caía no `data/` do próprio clone — ilha sem `wallet_annotations.json`, que é exatamente "o responsável não aparece pra mim". A busca custa 0,03s e só roda quando o caminho canônico falha. `custodian_upload.py` (ControleUpload.xlsx) passou a derivar a raiz da biblioteca da MESMA resolução (`resolver_raiz_biblioteca`), em vez de repetir o literal.
> - **Rodapé de diagnóstico sempre visível** (`GET /api/diagnostico-dados` + `static/js/controle_cargas/diagnostico.js`, novos): pasta dos dados e se é a compartilhada, quando `wallet_annotations.json` foi gravado pela última vez, quantos comentários/anotações vieram do disco, **quantas anotações valem na data de referência em tela** e de qual pasta o CÓDIGO subiu. Antes, esse diagnóstico só existia na 1ª linha do log de boot — que vai pra um arquivo, com a janela do servidor oculta.
> - **A semântica da anotação NÃO mudou** (decisão do usuário): continua gravada e lida por data de referência; o rodapé só torna óbvio quando a tela está numa data sem anotação.
